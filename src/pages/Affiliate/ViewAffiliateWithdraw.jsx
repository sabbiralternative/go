...
const ViewAffiliateWithdraw = () => {
  ...
  const { data: affiliateWithdrawData, isSuccess } = useAffiliateQuery({
    type: "view_affiliate_withdraw",
    affiliate_id,
    page,
  ...
  const meta = affiliateWithdrawData?.pagination;
  return (
    <Fragment>
      {/* Header */}
      <PageHeader title="View Affiliate Withdraw" />
      ...
      {/* Client Card */}
      {affiliateWithdrawData?.result?.map((affiliate, i) => {
        return (
      ...
      {isSuccess && affiliateWithdrawData?.result?.length === 0 && (
        <div className="client-card">
          <p style={{ fontSize: "11px" }}>No affiliate withdraw found.</p>
        </div>
      )}
    </Fragment>
  );
...
export default ViewAffiliateWithdraw;

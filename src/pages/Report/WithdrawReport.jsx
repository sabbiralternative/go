...
const WithdrawReport = () => {
  ...
  const { mutate, data: withdrawData, isSuccess } = useExportMutation();
  ...
  if (withdrawData?.result) {
    for (let data of withdrawData.result) {
  ...
  return (
    <Fragment>
      ...
      {/* Card */}
      {isSuccess && withdrawData?.result?.length > 0 && (
        <Fragment>
          <p
            style={{
              marginTop: "12px",
          ...
          >
            <span> Withdraw Count: {withdrawData?.result?.length}</span>
          </p>

          {withdrawData?.result?.map((item, i) => {
            return (
